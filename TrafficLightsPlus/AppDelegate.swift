...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func configureStatusItem() {
        ...
        item.button?.image = NSImage(systemSymbolName: "macwindow.on.rectangle", accessibilityDescription: "Traffic Lights Plus")

        let menu = NSMenu()
        menu.delegate = self
        menu.addItem(withTitle: I18n.string("app.menu.settings"), action: #selector(showSettings), keyEquivalent: ",").target = self
        menu.addItem(NSMenuItem.separator())
        let toggle = NSMenuItem(title: I18n.string("app.menu.enabled"), action: #selector(toggleEnabled(_:)), keyEquivalent: "")
        toggle.tag = MenuTag.enabled
        toggle.target = self
        toggle.state = preferences.enabled ? .on : .off
        menu.addItem(toggle)
        menu.addItem(NSMenuItem.separator())
        menu.addItem(withTitle: I18n.string("app.menu.quit"), action: #selector(quit), keyEquivalent: "q").target = self
        item.menu = menu
        statusItem = item
    }
    ...
    @objc func showSettings() {
        ...
        if settingsWindow == nil {
            ...
            window.title = I18n.string("app.settingsWindow.title")
        ...
        if let window = settingsWindow {
            tracker?.setSettingsWindowID(CGWindowID(window.windowNumber))
        }
